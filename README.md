...
```toml
[dependencies]
sabine = { git = "https://github.com/Lantharos/Sabine", tag = "v0.27" }
```

```sh
cargo install --git https://github.com/Lantharos/Sabine --tag v0.27 sabine-cli
```

For the TypeScript helpers used by the web UI:

```sh
bun add github:Lantharos/Sabine#v0.27
```
...
Public Sabine versions use `MAJOR.BUILD`, so this source tree is `0.27`. Cargo and npm encode the same
release as `0.27.0` because their package formats require three components. Build releases remain
...
Sabine 0.27 requires apps built with Sabine 0.23 or newer. Rebuild and redistribute older apps before
